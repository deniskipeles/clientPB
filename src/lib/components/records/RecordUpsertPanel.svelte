<script>
	...
	import { addSuccessToast, addErrorToast } from '$lib/stores/toasts';
	import Field from '$lib/components/base/Field.svelte';
	import Toggler from '$lib/components/base/Toggler.svelte';
	import ModelDateIcon from '$lib/components/base/ModelDateIcon.svelte';
	import OverlayPanel from '$lib/components/base/OverlayPanel.svelte';
	import AuthFields from '$lib/components/records/fields/AuthFields.svelte';
	import TextField from '$lib/components/records/fields/TextField.svelte';
	import NumberField from '$lib/components/records/fields/NumberField.svelte';
	import BoolField from '$lib/components/records/fields/BoolField.svelte';
	import EmailField from '$lib/components/records/fields/EmailField.svelte';
	import UrlField from '$lib/components/records/fields/UrlField.svelte';
	import DateField from '$lib/components/records/fields/DateField.svelte';
	import SelectField from '$lib/components/records/fields/SelectField.svelte';
	import JsonField from '$lib/components/records/fields/JsonField.svelte';
	import FileField from '$lib/components/records/fields/FileField.svelte';
	import RelationField from '$lib/components/records/fields/RelationField.svelte';
	import EditorField from '$lib/components/records/fields/EditorField.svelte';
	import ExternalAuthsList from '$lib/components/records/ExternalAuthsList.svelte';
	...
	$: hasFileChanges =
		...
		CommonHelper.hasNonEmptyProps(deletedFileNamesMap);
	...
	async function resolveModel(model) {
		// const displayFields = (collection?.schema?.filter((f) => f.type == 'relation' )?.map((f) => f.name) || [])?.join(',');
	...
	async function load(model) {
		...
		if (typeof model !== 'string') {
			collection_ = CommonHelper.getTableFromViews($page.data?.tables ?? [], collection.id);
			if (collection?.id != collection_?.id) {
				model = model?.id;
			}
		}
		...
		original =
			typeof model === 'string'
				? { id: model, collectionId: collection_?.id, collectionName: collection_?.name }
				: model || {};
	...
	async function replaceOriginal(newOriginal) {
		...
		const skipFields =
			collection?.schema?.filter((f) => f.type != 'file')?.map((f) => f.name) || [];
	...
	<svelte:fragment slot="header">
		...
		{:else}
			<h4 class="panel-title">
				{isNew ? 'New' : 'Edit'}
				<strong>{collection?.name?.split('_')?.join(' ') ?? ''}</strong> record
			</h4>
	...
	<div class="tabs-content no-animations">
		...
		>
			...
			<Field class="form-field {!isNew ? 'readonly' : ''}" name="id" let:uniqueId>
				...
				{/if}
				<input value="id/created/updated are auto" readonly />
				<input
					type="text"
					id={uniqueId}
