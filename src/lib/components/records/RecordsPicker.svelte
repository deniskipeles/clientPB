<script>
    ...
    $: collection = $page.data?.tables?.find((c) => c.id == collectionId) || null;
...
<OverlayPanel bind:this={pickerPanel} popup class="overlay-panel-xl" on:hide on:show {...$$restProps}>
    ...
    <p>The latest {batchSize} records are fetched. 
        If what you are looking for is not among it, then search to fetch desired records.</p>
