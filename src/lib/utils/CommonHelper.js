...
export default class CommonHelper {
	 ...
	 * @param  {String} viewOrTableId
	 * @return {RecordModel?}
	 */
	static getTableFromList(viewsAndTables, viewOrTableId) {
		try {
			return viewsAndTables.find(
				(element) => element.id == viewOrTableId || element['name'] == viewOrTableId
			);
		} catch (e) {
			return null;
		}
	 ...
	 * @return {RecordModel[]?}
	 */
	static getAuthTablesFromViews(viewsAndTables) {
		try {
			return viewsAndTables.filter((element) => element['type'] == 'auth');
		} catch (e) {
			return null;
		}
	}
	 ...
	 * @return {RecordModel[]?}
	 */
	static getViewsTables(viewsAndTables) {
		try {
			return viewsAndTables.filter((element) => element['type'] == 'view');
		} catch (e) {
			return null;
		}
	}
	 ...
	 * @return {RecordModel[]?}
	 */
	static getTables(viewsAndTables) {
		try {
			return viewsAndTables.filter((element) => element['type'] == 'base');
		} catch (e) {
			return null;
		}
	}
	 ...
	 * @return {RecordModel?}
	 */
	static getTableFromViews(viewsAndTables, viewOrTableId) {
		try {
			var currentView = getTableFromList(viewsAndTables, viewOrTableId);
			var currentAuths = getAuthTablesFromViews(viewsAndTables);
			// print('viewName');
			if (currentView != null && currentAuths != null) {
				var viewName = currentView['name'];
				// print(viewName);
				var authNames = currentAuths.map((e) => e['name']);
				for (var authName in authNames) {
					if (viewName.includes(`${authName}_view_`)) {
						// print(viewName);
						var modifyViewName = viewName.replace(`${authName}_view_`, '');
						// print(modifyViewName);
						return getTableFromList(viewsAndTables, modifyViewName);
					}
				}
			}
			return currentView;
	...
	static getApiExampleUrl(fallback) {
		let url =
			window.location.href.substring(0, window.location.href.indexOf('/_')) || fallback || '/';
